  ...
  try {
    ...
    for (const group of res.rows) {
      ...
      const userDetails = await client.query(
        'SELECT id, neon_user_id FROM users WHERE id = ANY($1)',
        [ids]
      );
      ...
      console.log(
        `Merging users [${sourceUserIds}] into target user ${targetUser.id} (${lower_email})`
      );
      ...
      for (const table of tables) {
        if (table === 'company_settings') {
          ...
          const targetSettings = await client.query(
            'SELECT id FROM company_settings WHERE user_id = $1',
            [targetUser.id]
          );
          if (targetSettings.rows.length > 0) {
            await client.query('DELETE FROM company_settings WHERE user_id = ANY($1)', [
              sourceUserIds,
            ]);
          } else {
            await client.query('UPDATE company_settings SET user_id = $1 WHERE user_id = ANY($2)', [
              targetUser.id,
              sourceUserIds,
            ]);
            // If multiple sources had settings, this might still fail. So let's be safer:
            await client.query(
              'DELETE FROM company_settings WHERE id NOT IN (SELECT MIN(id) FROM company_settings GROUP BY user_id)'
            );
          }
        } else {
          await client.query(`UPDATE ${table} SET user_id = $1 WHERE user_id = ANY($2)`, [
            targetUser.id,
            sourceUserIds,
          ]);
        }
